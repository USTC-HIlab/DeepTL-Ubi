...
species1 = 'MOUSE' #target species
species2 = 'HUMAN'
window_size = 31
phase = 'train'
domain_adaptation_task = 'HUMAN_MOUSE'
...
#         print('loss2 is', loss2)
        ...
        train_result = model.evaluate([X2,X2], [y2,y2])
        loss_train.append(train_result)
        ...
        ax.legend(loc='upper right', shadow=True)
        plt.title('loss of densenet {:s} epoch-{:d}'.format(species1, nb_epoch+nb_epoch_fine))
        plt.xlabel('epoch')
        plt.savefig(mainfolder+'/loss of epoch {:d}.png'.format(nb_epoch+nb_epoch_fine))
    ...
    for e in range(nb_epoch_fine):
        ...
        Out = model.predict([X_test, X_test])
        np.savetxt(prefolder+ '/prediction scores of epoch-{:d}.txt'.format(nb_epoch+e+1),Out[0])
        test_result = model.evaluate([X_test,X_test], [y_test,y_test])
        loss_test.append(test_result)
        np.savetxt(mainfolder+'/test_loss.txt',loss_test)
        ...
        dense1_output = dense1_layer_model.predict([X_test,X_test], batch_size=None) 
        print('zhongjianceng')
        save_feature(dense1_output,'independent',e+nb_epoch)
        fig, ax = plt.subplots()
        x = range(0, index_fine+nb_epoch)
        print(len(x))
        
        loss_train_arr = np.array(loss_train)
        print(np.shape(loss_train_arr))
        loss_valid_arr = np.array(loss_valid)
        loss_test_arr = np.array(loss_test)
        plt.plot(x, loss_train_arr[:,0], 'r-', label='train loss ')
        ax.legend(loc='upper right', shadow=True)
        ...
        ax.legend(loc='lower right', shadow=True)
        plt.title('test of {:s}'.format(species1))
        plt.savefig(figfolder+'/ptm-{:s}-epoch-{:d}-auc-{:.4f}-acc-{:.4f}-test.png'.format(species1, e+1+nb_epoch, auc, Acc))
        plt.close()
...
if __name__ == '__main__':
    ...
    os.environ['CUDA_VISIBLE_DEVICES'] = '3'  # '1' TITAN 1080
    weigth = 0.8
    nb_epoch = 30
    nb_epoch_fine = 100
    window_size = 31
